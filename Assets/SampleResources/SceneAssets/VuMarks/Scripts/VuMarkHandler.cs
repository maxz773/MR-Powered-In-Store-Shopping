...
public class VuMarkHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (var vuMarkBehaviour in FindObjectsByType<VuMarkBehaviour>(FindObjectsSortMode.None))
            ToggleRenderers(vuMarkBehaviour.gameObject, false);

        mNearestVuMarkScreenPanel = FindFirstObjectByType<VuMarkObserverStatusUI>();
    ...
    {
        var canvasImage = vuMarkBehaviour.gameObject.GetComponentsInChildren<UnityEngine.UI.Image>().First();
        ...
        var vuMarkDataType = GetVuMarkDataType(vuMarkBehaviour);
        var vuMarkDesc = GetNumericVuMarkDescription(vuMarkBehaviour);
        ...
        var text =
            ...
            "\n" + vuMarkDataType;
        SampleUtil.AssignStringToTextComponent(vuMarkBehaviour.gameObject, text);
